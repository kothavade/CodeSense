...
// NOTE: Microsoft specifiers to tell compiler that symbols are imported/exported from a .dll
        ...
        #endif
        #define  __declspec(dllexport)     // We are building the library as a Win32 shared library (.dll)
    #elif defined(USE_LIBTYPE_SHARED)
        #define  __declspec(dllimport)     // We are using the library as a Win32 shared library (.dll)
    #endif
#endif

#ifndef 
    #define        // Functions defined as 'extern' by default (implicit specifiers)
#endif
...
// Basic shapes drawing functions
 ...
 void DrawLineBSpline(Vector2 *points, int pointCount, float thick, Color color);                   // Draw a B-Spline line, minimum 4 points
 void DrawLineCatmullRom(Vector2 *points, int pointCount, float thick, Color color);                // Draw a Catmull Rom spline line, minimum 4 points
